...
const START_URL = "https://www.dema-argentina.com.ar/home";

const EXCEL_PATH = path.join(process.cwd(), "data", "productos.xlsx");
const IMAGE_DIR = path.join(process.cwd(), "public", "products", "dema");
const IMAGE_PUBLIC_PREFIX = "/products/dema";
...
async function getCategoryLinks(page: any): Promise<CategoryTarget[]> {
  ...
  const unique = [...new Set(
    links
      .filter((href) => href.includes("/categoria-producto/"))
  ...
  for (const url of unique) {
    ...
    // 👇 acá generamos categoria y subcategoria automáticamente
    let categoria = "DEMA";
    let subcategoria = key.replace(/-/g, " ");

    if (key.includes("contencion")) categoria = "Contención";
    else if (key.includes("ayudas")) categoria = "Ayudas diarias";
    else if (key.includes("media")) categoria = "Medias Compresión";
    else if (key.includes("motion")) categoria = "In Motion";
    else if (key.includes("entrenamiento")) categoria = "Entrenamiento";
...
async function getProductLinksFromCategory(page: any, categoryUrl: string) {
  ...
  await page.goto(currentUrl, {
    ...
    timeout: 60000, // bajamos un poco
  });
} catch (err) {
  console.warn("⚠️ Error cargando categoría:", currentUrl);
  break; // salta esta categoría
}
    await page.waitForTimeout(2000);

    const productLinks = await page.$$eval("a", (anchors: HTMLAnchorElement[]) =>
      anchors.map((a) => a.href).filter(Boolean)
    );

    productLinks
      .filter((href) => href.includes("dema-argentina.com.ar/producto/"))
      .forEach((href) => found.add(href.split("?")[0]));

    const nextLink = await page
      .$eval(
        "a.next, .next a, a.page-numbers.next",
        (a: HTMLAnchorElement) => a.href
      )
      .catch(() => "");

    if (!nextLink || nextLink === currentUrl) break;
...
for (const category of categories) {
  catIndex++;
  console.log(
    `\n📂 [${catIndex}/${categories.length}] ${category.categoria} > ${category.subcategoria}`
  );
